...
class SklearnTrainer:
    ...
    def train(
        ...
        if strategy == "cross_validation":
            return self._train_cv(
                X, y, algorithm, task_type, hyperparams, algo_info,
                n_folds, shuffle, random_state, target_column, model_name,
            )
        else:
            return self._train_holdout(
                X, y, algorithm, task_type, hyperparams, algo_info,
                test_size, shuffle, random_state, target_column, model_name,
            )

    def _train_holdout(
        self, X, y, algorithm, task_type, hyperparams, algo_info,
        test_size, shuffle, random_state, target_column, model_name=None,
    ) -> Dict[str, Any]:
        """Entrenamiento con holdout train/test split."""
        X_train, X_test, y_train, y_test = train_test_split(
            X, y, test_size=test_size, random_state=random_state,
            shuffle=shuffle,
            stratify=y if task_type == "classification" and shuffle and len(y.unique()) > 1 else None,
        )
        ...
        logger.info("Holdout training completo: run_id=%s, metrics=%s", mlflow_run_id, metrics)
    ...
    def _train_cv(
        self, X, y, algorithm, task_type, hyperparams, algo_info,
        n_folds, shuffle, random_state, target_column, model_name=None,
    ) -> Dict[str, Any]:
        ...
        logger.info(
            ...
            n_folds, X.shape[1], len(X),
        )

        run_name = f"{model_name} (CV-{n_folds})" if model_name else f"{algo_info['display_name']} (CV-{n_folds})"
        with mlflow.start_run(run_name=run_name) as run:
            mlflow.set_tags({
                "framework": "sklearn",
                "algorithm": algorithm,
        ...
        logger.info("CV training completo: run_id=%s, metrics=%s", mlflow_run_id, metrics)
...
class PyTorchTrainer:
        ...
        try:
            model = ModelFactory.get_model(
                ...
                **{k: v for k, v in hyperparams.items()
                   if k not in ("in_channels", "num_classes", "epochs", "learning_rate",
                                "batch_size", "optimizer", "scheduler", "weight_decay",
                                "hidden_layers", "dropout", "activation")},
            )
            return model
        except ValueError:
            raise ValueError(
    ...
    def train(
        ...
        if strategy == "cross_validation":
            return self._train_pytorch_cv(
                X, X_np, y_np, algorithm, task_type, hyperparams, label_map,
                output_dim, n_folds, shuffle, random_state, target_column, model_name,
            )
        ...
        logger.info(
            ...
            len(X_train), len(X_test), input_dim, output_dim,
        )
        ...
        # Modelo
        model = self._get_model(algorithm, input_dim, output_dim, task_type, hyperparams)
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            if algorithm == "mlp":
                mlflow.log_params({
                    "hidden_layers": str(hyperparams.get("hidden_layers", [128, 64])),
                    "dropout": hyperparams.get("dropout", 0.2),
                    "activation": hyperparams.get("activation", "relu"),
                })
            ...
            for epoch in range(epochs):
                ...
                if epoch % max(1, epochs // 10) == 0 or epoch == epochs - 1:
                    mlflow.log_metrics({"train_loss": epoch_loss, "epoch": epoch}, step=epoch)
            ...
            try:
                ...
                ts_path = models_dir / f"pytorch_{algorithm}_{run.info.run_id[:8]}_scripted.pt"
        ...
        logger.info(
            ...
            algorithm, mlflow_run_id, metrics,
        )
    ...
    def _train_pytorch_cv(
        self, X_df, X_np, y_np, algorithm, task_type, hyperparams, label_map,
        output_dim, n_folds, shuffle, random_state, target_column, model_name=None,
    ) -> Dict[str, Any]:
        ...
        mlflow.pytorch.autolog(log_models=True)
        algo_display = algorithm.upper() if algorithm != "mlp" else "MLP"
        run_name = f"{model_name} (CV-{n_folds})" if model_name else f"PyTorch {algo_display} (CV-{n_folds})"
        ...
        with mlflow.start_run(run_name=run_name) as run:
            ...
            for fold_idx, (train_idx, val_idx) in enumerate(
                cv.split(X_np, split_target) if split_target is not None else cv.split(X_np)
            ):
                ...
                fold_model = self._get_model(algorithm, input_dim, output_dim, task_type, hyperparams)
                ...
                fold_metrics = self._evaluate_pytorch(fold_model, val_loader, task_type, criterion)
            ...
            final_model = self._get_model(algorithm, input_dim, output_dim, task_type, hyperparams)
            final_model = final_model.to(self.device)
            criterion = nn.CrossEntropyLoss() if task_type == "classification" else nn.MSELoss()
            optimizer = torch.optim.Adam(final_model.parameters(), lr=hyperparams.get("learning_rate", 0.001))
        ...
        logger.info("PyTorch CV training completo: run_id=%s, metrics=%s", mlflow_run_id, metrics)
    ...
    def _prepare_features(self, X: pd.DataFrame) -> pd.DataFrame:
        """Preparar features numéricas para PyTorch."""
        X_clean = X.copy()
        for col in X_clean.select_dtypes(include=["object", "category"]).columns:
            X_clean[col] = X_clean[col].astype("category").cat.codes
        for col in X_clean.select_dtypes(include=["number"]).columns:
