...
class SegCalibratedMCDropout:
    ...
    def optimize_parameters(self) -> tuple[float, float, float]:
        ...
        for p in self.p_values:
            ...
            all_log_probs = torch.cat(log_probs_list, dim=1)
            avg_log_probs = torch.mean(all_log_probs, dim=0)
            nll = F.nll_loss(avg_log_probs, targets).item()
            
            mc_preds = torch.cat(mc_preds_list, dim=1)
    ...
    def _find_relaxed_scale(self, probs: torch.Tensor, entropy: torch.Tensor, 
        ...
        """Adjust scale using bisection to balance scaled uncertainty and Hamming error rate."""
        # Compute predictions and error rate
        preds = torch.argmax(probs, dim=1)  # [B, H, W]
        error = (preds != targets).float()  # [B, H, W]
        error_rate = error.mean().item()
        mean_entropy = entropy.mean().item()

        # Function to compute balance for a given scale
        def compute_balance(scale_candidate: float) -> float:
            scaled_uncertainty = entropy * scale_candidate
            mean_scaled = scaled_uncertainty.mean().item()
            return mean_scaled - error_rate

        # Handle edge case where mean entropy is near zero
        if mean_entropy < EPS:
            return init_scale  # Avoid division by zero
        ...
        # Expand high until compute_balance(high) is positive
        while compute_balance(high) < 0 and high < 1e6:
            high *= 2
