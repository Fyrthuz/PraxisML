...
def save_outputs(images, masks, mean_prediction, uncertainty, method_dir, sample_idx):
    # [Unchanged from original code]
    os.makedirs(method_dir, exist_ok=True)
    if isinstance(images, list) and len(images) > 0:
        save_image(images[0], os.path.join(method_dir, f"sample_{sample_idx}.png"))
    if isinstance(masks, list) and len(masks) > 0:
        save_image(masks[0], os.path.join(method_dir, f"mask_{sample_idx}.png"))
    save_image(mean_prediction, os.path.join(method_dir, f"mean_prediction_{sample_idx}.png"))
    save_image(uncertainty, os.path.join(method_dir, f"uncertainty_{sample_idx}.png"))
...
if __name__ == "__main__":
    ...
    for directory in config.get("directories", []):
        os.makedirs(os.path.join(SAVE_PATH, directory), exist_ok=True)
    ...
    model = torch.hub.load('mateuszbuda/brain-segmentation-pytorch',
                           ...
                           init_features=config_model["init_features"],
                           pretrained=config_model["pretrained"]).to(device)
    print("Model loaded.")
    ...
    for idx, (img_path, mask_path) in enumerate(pairs):
        ...
        save_image(normal_mask, os.path.join(SAVE_PATH, "original", f"normal_mask_{idx}.png"))
        ...
        mc_dropout.enable(ignore_type_layers=[torch.nn.ReLU, torch.nn.Softmax])
        ...
        save_outputs(mc_images, mc_masks, mc_mean_prediction, mc_entropy, os.path.join(SAVE_PATH, "mc_dropout"), idx)
        ...
        save_outputs(tta_images, tta_masks, tta_mean_prediction, tta_entropy, os.path.join(SAVE_PATH, "tta"), idx)
        ...
        save_outputs(noise_images, noise_masks, noise_mean_prediction, noise_entropy, os.path.join(SAVE_PATH, "noisy"), idx)
        ...
        os.makedirs(os.path.join(SAVE_PATH, 'base'), exist_ok=True)
        save_image(consensus_prob, os.path.join(SAVE_PATH, 'base', f"consensus_probability_{idx}.png"))
        save_image(fusion_mask, os.path.join(SAVE_PATH, 'base', f"consensus_binary_{idx}.png"))
        save_image(consensus_uncertainty, os.path.join(SAVE_PATH, 'base', f"consensus_uncertainty_{idx}.png"))
        ...
        os.makedirs(os.path.join(SAVE_PATH, 'refined'), exist_ok=True)
        save_image(final_segmentation, os.path.join(SAVE_PATH, 'refined', f"final_segmentation_{idx}.png"))
        save_image(final_uncertainty, os.path.join(SAVE_PATH, 'refined', f"final_uncertainty_{idx}.png"))
    ...
    metrics = ['iou', 'dice', 'nll', 'ece', 'brier', 'accuracy', 'precision', 'recall']
    ...
    csv_path = os.path.join(SAVE_PATH, 'metrics_summary.csv')
    df.to_csv(csv_path)
    ...
    plt.tight_layout()
    plot_path = os.path.join(SAVE_PATH, 'metrics_summary.png')
    ...
    plt.tight_layout()
    enhanced_plot_path = os.path.join(SAVE_PATH, 'enhanced_metrics_comparison.png')
    ...
    # Adjust layout and save the plot
    plt.tight_layout()
    box_plot_path = os.path.join(SAVE_PATH, 'box_plot_comparison.png')
    ...
    # for sample, metrics in overall_metrics.items():
    #     print(f"{sample}:")
